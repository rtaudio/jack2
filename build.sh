...
if $HAVE_NEON; then
	echo "NEON found"
	HARD_OPT="-O3 -fomit-frame-pointer -ffast-math -funroll-all-loops -ftree-vectorize -mfloat-abi=hard -mfpu=neon -DARM_HPET" # `xeno-config --skin=posix --cflags`"
else
	echo "NEON NOT found"
...
CXXFLAGS=$HARD_OPT CFLAGS=$HARD_OPT ./waf configure --alsa --portaudio --prefix=/usr
